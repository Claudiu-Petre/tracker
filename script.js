...
// EXPERIMENT: see if constructors work!
...
class App {
    ...
    // open street map - is an open source map for free
    L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
    }).addTo(this.#map);
    
    // clicks so the marker popups on the map + content
    ...
    _newWorkout(e){
        const validInputs = (...inputs) => inputs.every(inp => Number.isFinite(inp));// helper function - checks if inputs are numbers
        const allPositives = (...inputs) => inputs.every(inp => inp > 0);//helper function - checks if numbers are positives
        e.preventDefault();
        ...
        //Check if data is valid using a guard close.
        if(
    ...
    _setLocalStorage(){     //localStorage is an API provided by the browser
        localStorage.setItem('workouts', JSON.stringify(this.#workouts));  //converts an OBJECT to a STRING
    }
    _getLocalStorage(){
